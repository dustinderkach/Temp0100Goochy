...
import { getTemp009Goochy as getTemp009Goochy } from "../../../src/services/temp009Goochy/GetTemp009Goochy";
...
describe("GetTemp009Goochy test suite", () => {
	const ddbClientMock = {
		send: jest.fn(),
	};
	...
	test("should return temp009Goochy if no queryStringParameters", async () => {
		ddbClientMock.send.mockResolvedValueOnce(someItems);
		const getResult = await getTemp009Goochy({} as any, ddbClientMock as any);
	...
	test("should return 400 if no id in queryStringParameters", async () => {
		const getResult = await getTemp009Goochy(
			{
	...
	test("should return 404 if no id in queryStringParameters", async () => {
		...
		const expectedResult = {
			statusCode: 404,
			body: JSON.stringify(`Temp009Goochy with id 123 not found!`),
		};
		expect(getResult).toEqual(expectedResult);
	});
