...
interface Temp009GoochyAuthStackProps extends StackProps {
	envName: string;
	photosBucket: IBucket;
	env: { account: string; region: string };
}

export class Temp009GoochyAuthStack extends Stack {
	public userPool: UserPool;
	private props: Temp009GoochyAuthStackProps;
	...
	constructor(
		scope: Construct,
		id: string,
		props: Temp009GoochyAuthStackProps
	) {
	...
	private createUserPool() {
		this.userPool = new UserPool(
			this,
			`${this.props.envName}-Temp009GoochyUserPool`,
			{
		...
		new CfnOutput(this, `${this.props.envName}-Temp009GoochyUserPoolId`, {
	...
	private createUserPoolClient() {
		this.userPoolClient = this.userPool.addClient(
			`${this.props.envName}-Temp009GoochyUserPoolClient`,
			{
				authFlows: {
		...
		new CfnOutput(
			this,
			`${this.props.envName}-Temp009GoochyUserPoolClientId`,
			{
	...
	private createAdminsGroup() {
		new CfnUserPoolGroup(
			this,
			`${this.props.envName}-Temp009GoochyAdmins`,
			{
	...
	private createIdentityPool() {
		this.identityPool = new CfnIdentityPool(
			this,
			`${this.props.envName}-Temp009GoochyIdentityPool`,
			{
		...
		new CfnOutput(
			this,
			`${this.props.envName}-Temp009GoochyIdentityPoolId`,
			{
