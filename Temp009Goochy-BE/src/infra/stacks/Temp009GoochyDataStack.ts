...
interface Temp009GoochyDataStackProps extends StackProps {
	allRegions: string[];
	envName: string;
}

export class Temp009GoochyDataStack extends Stack {
	...
	constructor(
		scope: Construct,
		id: string,
		props: Temp009GoochyDataStackProps
	) {
		...
		this.primaryTable = new dynamodb.TableV2(
			this,
			`${props.envName}-Temp009GoochyTable`,
			{
				partitionKey: {
					name: "id",
				...
				tableName: `${props.envName}-Temp009GoochyTable`,
		...
		props.allRegions.forEach((region) => {
			...
			new CfnOutput(
				this,
				`${props.envName}-Temp009GoochyTableName-${region}`,
				{
			...
			new CfnOutput(
				this,
				`${props.envName}-Temp009GoochyTableArn-${region}`,
				{
					value: arn,
