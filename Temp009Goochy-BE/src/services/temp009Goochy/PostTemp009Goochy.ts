...
import { validateAsTemp009GoochyEntry } from "../shared/Validator";
...
export async function postTemp009Goochy(
	...
	item.id = randomId;
	validateAsTemp009GoochyEntry(item);
    // Fetch the bucket ARN from SSM Parameter Store
    const parameterName = `/${process.env.ENV_NAME}/Temp009GoochyAdminPhotosBucketArn`;
