...
interface Temp010GoochyDataStackProps extends StackProps {
	allRegions: string[];
	envName: string;
}

export class Temp010GoochyDataStack extends Stack {
	...
	constructor(
		scope: Construct,
		id: string,
		props: Temp010GoochyDataStackProps
	) {
		...
		this.primaryTable = new dynamodb.TableV2(
			this,
			`${props.envName}-Temp010GoochyTable`,
			{
				partitionKey: {
					name: "id",
				...
				tableName: `${props.envName}-Temp010GoochyTable`,
		...
		props.allRegions.forEach((region) => {
			...
			new CfnOutput(
				this,
				`${props.envName}-Temp010GoochyTableName-${region}`,
				{
			...
			new CfnOutput(
				this,
				`${props.envName}-Temp010GoochyTableArn-${region}`,
				{
					value: arn,
