...
export class Temp010GoochyUiDeploymentStack extends Stack {
	constructor(
		scope: Construct,
		id: string,
		...
		const deploymentBucket = new Bucket(this, "uiDeploymentBucket", {
			bucketName: `${props.envName.toLowerCase()}-temp010goochy-fe-${suffix}`,
		});
		...
		new BucketDeployment(
			this,
			`${props.envName.toLowerCase()}Temp010GoochyDeployment`,
			{
		...
		const distribution = new Distribution(
			this,
			`${props.envName.toLowerCase()}Temp010GoochyDistribution`,
			{
		...
		new CfnOutput(
			this,
			`${props.envName.toLowerCase()}Temp010GoochyDistributionDomainName`,
			{
