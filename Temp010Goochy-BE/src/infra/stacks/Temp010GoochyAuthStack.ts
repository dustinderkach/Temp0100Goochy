...
interface Temp010GoochyAuthStackProps extends StackProps {
	envName: string;
	photosBucket: IBucket;
	env: { account: string; region: string };
}

export class Temp010GoochyAuthStack extends Stack {
	public userPool: UserPool;
	private props: Temp010GoochyAuthStackProps;
	...
	constructor(
		scope: Construct,
		id: string,
		props: Temp010GoochyAuthStackProps
	) {
	...
	private createUserPool() {
		this.userPool = new UserPool(
			this,
			`${this.props.envName}-Temp010GoochyUserPool`,
			{
		...
		new CfnOutput(this, `${this.props.envName}-Temp010GoochyUserPoolId`, {
	...
	private createUserPoolClient() {
		this.userPoolClient = this.userPool.addClient(
			`${this.props.envName}-Temp010GoochyUserPoolClient`,
			{
				authFlows: {
		...
		new CfnOutput(
			this,
			`${this.props.envName}-Temp010GoochyUserPoolClientId`,
			{
	...
	private createAdminsGroup() {
		new CfnUserPoolGroup(
			this,
			`${this.props.envName}-Temp010GoochyAdmins`,
			{
	...
	private createIdentityPool() {
		this.identityPool = new CfnIdentityPool(
			this,
			`${this.props.envName}-Temp010GoochyIdentityPool`,
			{
		...
		new CfnOutput(
			this,
			`${this.props.envName}-Temp010GoochyIdentityPoolId`,
			{
