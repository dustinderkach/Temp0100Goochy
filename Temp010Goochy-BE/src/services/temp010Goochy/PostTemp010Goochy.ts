...
import { validateAsTemp010GoochyEntry } from "../shared/Validator";
...
export async function postTemp010Goochy(
	...
	item.id = randomId;
	validateAsTemp010GoochyEntry(item);
    // Fetch the bucket ARN from SSM Parameter Store
    const parameterName = `/${process.env.ENV_NAME}/Temp010GoochyAdminPhotosBucketArn`;
