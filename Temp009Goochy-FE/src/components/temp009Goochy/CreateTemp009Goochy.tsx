...
export default function CreateTemp009Goochy({ dataService }: CreateTemp009GoochyProps) {
	...
	const handleSubmit = async (event: SyntheticEvent) => {
		...
		if (name && location) {
			const id = await dataService.createTemp009Goochy(
				name,
				location,
				photo
			);
			setActionResult(`Created temp009Goochy with id ${id}`);
			setName("");
			setLocation("");
		} else {
	...
	function renderForm() {
		...
		return (
			<form onSubmit={(e) => handleSubmit(e)}>
				...
				{renderPhoto()}
				<br />
				<input type="submit" value="Create temp009Goochy" />
			</form>
		);
	}
