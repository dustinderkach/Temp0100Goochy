...
export default function CreateTemp010Goochy({ dataService }: CreateTemp010GoochyProps) {
	...
	const handleSubmit = async (event: SyntheticEvent) => {
		...
		if (name && location) {
			const id = await dataService.createTemp010Goochy(
				name,
				location,
				photo
			);
			setActionResult(`Created temp010Goochy with id ${id}`);
			setName("");
			setLocation("");
		} else {
	...
	function renderForm() {
		...
		return (
			<form onSubmit={(e) => handleSubmit(e)}>
				...
				{renderPhoto()}
				<br />
				<input type="submit" value="Create temp010Goochy" />
			</form>
		);
	}
